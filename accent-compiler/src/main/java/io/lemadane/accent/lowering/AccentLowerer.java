package io.lemadane.accent.lowering;
...
import io.lemadane.accent.lexer.TokenType;
import java.util.*;

public final class AccentLowerer implements ImportResolver {
    ...
    public AccentLowerer(SymbolTable symbolTable) {
    ...
    private void injectLoggerField(List<String> modifiers, List<Member> members, String className) {
        if (hasLoggingAnnotation(modifiers)) {
            TypeNode type = new BaseTypeNode("io.lemadane.accent.logging.Logger", List.of(), true);
            ...
            Expression logManager = new IdentifierExpr("io.lemadane.accent.logging.LogManager", null);
    ...
    private Expression lowerExpression(Expression expr) {
        if (expr instanceof LiteralExpr || expr instanceof IdentifierExpr || expr instanceof ThisExpr || expr instanceof SuperExpr) {
            ...
            // Lower: async expr -> AccentRuntime.async(() -> expr)
            ...
            return new MethodCallExpr(new IdentifierExpr("io.lemadane.accent.runtime.AccentRuntime", null), "async", List.of(), List.of(lambda), false, ae.token());
    ...
    private ResolvedType getTypeOf(Expression expr) {
        if (expr instanceof LiteralExpr lit) {
            ...
            return new ResolvedType(symbolTable.getType("io.lemadane.accent.runtime.AccentFuture"), true, List.of(type), 0);
        } else if (expr instanceof AwaitExpr awe) {
    ...
    private Expression lowerMarkupExpression(MarkupExpr me) {
        ...
        Parameter writerParam = new Parameter(new BaseTypeNode("io.lemadane.accent.html.HtmlWriter", List.of(), true), writerName, false, Optional.empty());
                ...
                if (matched != null) {
                    for (Parameter param : matched.parameters()) {
                        ...
                        if (paramType != null && paramType.info().fullName().equals("io.lemadane.accent.html.HtmlChildren")) {
                            ...
                            Parameter childWriterParam = new Parameter(new BaseTypeNode("io.lemadane.accent.html.HtmlWriter", List.of(), true), childWriterName, false, Optional.empty());
