package io.lemadane.accent.emitter;
...
public final class JavaEmitter {
    ...
    private void emitMember(Member member) {
        if (member instanceof FieldDecl fd) {
            ...
            if (md.body().isPresent()) {
                ...
                if (md.isGenerator()) {
                    printIndent();
                    sb.append("return io.lemadane.accent.runtime.AccentGenerator.of((__emit) -> {\n");
                    indentLevel++;
    ...
    private String emitExpression(Expression expr) {
        if (expr instanceof LiteralExpr lit) {
            ...
            return "io.lemadane.accent.json.Json.parse(" + sb.toString() + ", " + classLiteral + ")";
            ...
            return "((" + castType + ") io.lemadane.accent.sql.Sql.execute(" + sqlString + ", " + paramList + ", " + classLiteral + "))";
