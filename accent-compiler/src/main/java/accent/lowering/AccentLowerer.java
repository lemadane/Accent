...
public final class AccentLowerer implements ImportResolver {
    ...
    private void injectLoggerField(List<String> modifiers, List<Member> members, String className) {
        if (hasLoggingAnnotation(modifiers)) {
            TypeNode type = new BaseTypeNode("accent.logging.Logger", List.of(), true);
            ...
            Expression logManager = new IdentifierExpr("accent.logging.LogManager", null);
    ...
    private Expression lowerExpression(Expression expr) {
        if (expr instanceof LiteralExpr || expr instanceof IdentifierExpr || expr instanceof ThisExpr || expr instanceof SuperExpr) {
            ...
            return new MethodCallExpr(new IdentifierExpr("accent.runtime.AccentRuntime", null), "async", List.of(), List.of(lambda), false, ae.token());
    ...
    private ResolvedType getTypeOf(Expression expr) {
        if (expr instanceof LiteralExpr lit) {
            ...
            return new ResolvedType(symbolTable.getType("accent.runtime.AccentFuture"), true, List.of(type), 0);
        } else if (expr instanceof AwaitExpr awe) {
    ...
    private Expression lowerMarkupExpression(MarkupExpr me) {
        ...
        Parameter writerParam = new Parameter(new BaseTypeNode("accent.html.HtmlWriter", List.of(), true), writerName, false, Optional.empty());
                ...
                if (matched != null) {
                    for (Parameter param : matched.parameters()) {
                        ...
                        if (paramType != null && paramType.info().fullName().equals("accent.html.HtmlChildren")) {
                            ...
                            Parameter childWriterParam = new Parameter(new BaseTypeNode("accent.html.HtmlWriter", List.of(), true), childWriterName, false, Optional.empty());
