package io.lemadane.jatot.semantic;

import io.lemadane.jatot.ast.Ast.*;
import io.lemadane.jatot.diagnostic.Diagnostic;
import io.lemadane.jatot.diagnostic.DiagnosticSeverity;
import io.lemadane.jatot.lexer.Token;
import io.lemadane.jatot.symbol.SymbolTable;
import io.lemadane.jatot.symbol.SymbolTable.ResolvedType;
import io.lemadane.jatot.symbol.SymbolTable.TypeInfo;
import io.lemadane.jatot.symbol.SymbolTable.ConstructorInfo;
import io.lemadane.jatot.symbol.SymbolTable.ImportResolver;
import java.util.*;
...
public final class SemanticAnalyzer implements ImportResolver {
    ...
    private boolean hasLoggingAnnotation(List<String> modifiers) {
        ...
        if (modifiers.contains("@io.lemadane.jatot.logging.Logging")) {
            return true;
        }
        if (modifiers.contains("@Logging")) {
            if (currentUnit == null) return false;
            return currentUnit.imports().contains("io.lemadane.jatot.logging.Logging") || currentUnit.imports().contains("io.lemadane.jatot.logging.*");
        }
        return false;
    }
    ...
    private void injectLoggerScope(List<String> modifiers, List<Member> members, Token errorToken) {
        if (hasLoggingAnnotation(modifiers)) {
            ...
            TypeInfo loggerInfo = symbolTable.getType("io.lemadane.jatot.logging.Logger");
            if (loggerInfo == null) {
                ...
                loggerInfo = new TypeInfo(
                    "io.lemadane.jatot.logging.Logger", false, false, false, false, Optional.empty(), List.of(),
                    List.of(
    ...
    private void checkStatement(Statement stmt) {
        if (stmt instanceof BlockStmt bs) {
            ...
            if (ifs.condition() instanceof BinaryExpr bin && bin.operator().type() == io.lemadane.jatot.lexer.TokenType.BANG_EQUAL) {
            ...
            if (type != null && type.info().fullName().equals("io.lemadane.jatot.runtime.JatotFuture")) {
    ...
    private ResolvedType checkExpressionInternal(Expression expr) {
        if (expr instanceof LiteralExpr lit) {
            ...
            return new ResolvedType(symbolTable.getType("io.lemadane.jatot.html.Html"), true, List.of(), 0);
            ...
            if (bin.operator().type() == io.lemadane.jatot.lexer.TokenType.ASSIGN) {
                // Reassignment checks
            ...
            if (bin.operator().type() == io.lemadane.jatot.lexer.TokenType.NULL_COALESCING) {
                // left ?? right
            ...
            io.lemadane.jatot.lexer.TokenType opType = bin.operator().type();

            // Textual boolean-only operators: require boolean operands on both sides
            if (opType == io.lemadane.jatot.lexer.TokenType.AND  ||
                opType == io.lemadane.jatot.lexer.TokenType.OR   ||
                opType == io.lemadane.jatot.lexer.TokenType.NAND ||
                opType == io.lemadane.jatot.lexer.TokenType.NOR  ||
                opType == io.lemadane.jatot.lexer.TokenType.XOR  ||
                opType == io.lemadane.jatot.lexer.TokenType.XNOR) {
            ...
            if (opType == io.lemadane.jatot.lexer.TokenType.EQUAL_EQUAL || opType == io.lemadane.jatot.lexer.TokenType.BANG_EQUAL ||
                opType == io.lemadane.jatot.lexer.TokenType.LESS || opType == io.lemadane.jatot.lexer.TokenType.LESS_EQUAL ||
                opType == io.lemadane.jatot.lexer.TokenType.GREATER || opType == io.lemadane.jatot.lexer.TokenType.GREATER_EQUAL ||
                opType == io.lemadane.jatot.lexer.TokenType.AND_AND || opType == io.lemadane.jatot.lexer.TokenType.OR_OR) {
                return new ResolvedType(symbolTable.getType("boolean"), true, List.of(), 0);
            }
            ...
            if (un.operator().type() == io.lemadane.jatot.lexer.TokenType.NOT) {
                if (type != null && !isBooleanType(type)) {
                    error(un.operator(), "Operator 'not' requires a boolean operand, but found " + type.info().simpleName() + ".");
                }
                return new ResolvedType(symbolTable.getType("boolean"), true, List.of(), 0);
            }
            if (un.operator().type() == io.lemadane.jatot.lexer.TokenType.PLUS_PLUS || un.operator().type() == io.lemadane.jatot.lexer.TokenType.MINUS_MINUS) {
            ...
            TypeInfo fut = symbolTable.getType("io.lemadane.jatot.runtime.JatotFuture");
    ...
    private void checkMarkupNode(MarkupNode node) {
        if (node instanceof ElementNode el) {
            ...
            TypeInfo compInterfaceInfo = symbolTable.getType("io.lemadane.jatot.html.Component");
            if (compInterfaceInfo == null) {
                error(comp.token(), "Component interface io.lemadane.jatot.html.Component not found.");
                return;
            }
            ...
            for (ConstructorInfo c : constructors) {
                ...
                for (Parameter param : c.parameters()) {
                    ...
                    if (paramType != null && paramType.info().fullName().equals("io.lemadane.jatot.html.HtmlChildren")) {
